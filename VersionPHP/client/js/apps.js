...
class EventsManager {
    ...
    obtenerDataInicial() {
        ...
        $.ajax({ //SE OBTIENEN LOS EVENTOS
          ...
          success: (data) =>{
            ...
            if (data.msg=="OK") {               // SE LLENA EL CALENDARIO (poblarCalendario) 
               //console.log(data.eventos);
              this.poblarCalendario(data.eventos) /*getEvents.php me tiene que devolver un array $data con $data['eventos']*/
            }else {
    ...
    poblarCalendario(eventos) {
        $('.calendario').fullCalendar({
          ...
          timeFormat: 'H:mm',
          eventDrop: (event) => { /*que seria el parametro event de donde sale?*/
              this.actualizarEvento(event) 
          },
          events: eventos, /*que hace esto de aca??*/
          eventDragStart: (event,jsEvent) => { //y esto?
            $('.delete-btn').find('img').attr('src', "img/trash-open.png");
            $('.delete-btn').css('background-color', '#a70f19')
          },
          eventDragStop: (event,jsEvent) =>{ /*de donde salen estos parametros? que son cada uno?*/
    ...
    actualizarEvento(evento) {
        ...
        $.ajax({
          ...
          error: function(){
            alert("error en la comunicación con el servidor4");
          }
        })
    }
